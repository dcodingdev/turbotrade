 ...
 */
const uploadSingleToIK = async (file, folder) => {
    try {
...
export const uploadToImageKit = async (req, res, next) => {
    ...
    try {
        ...
        if (files["subImages"]?.length > 0) {
            const uploadPromises = files["subImages"].map((file) => uploadSingleToIK(file, "/products/sub"));
            fileData.subImages = await Promise.all(uploadPromises);
        }
        /**
         * Attach to request
        ...
        logger.error(`ImageKit Multi-Upload Error: ${error.message}`);
        res.status(500).json({
            success: false,
