...
import amqp from 'amqplib';
export const RMQ_NAMES = {
    getExchange: (domain, service, type = 'topic') => `${domain}.${service}.${type}`.toLowerCase(),
    getRoutingKey: (domain, entity, action) => `${domain}.${entity}.${action}`.toLowerCase(),
    getQueue: (domain, service, action, consumer) => `${domain}.${service}.${action}.${consumer}`.toLowerCase()
};
let connection = null;
let channel = null;
export const connectRMQ = async (uri) => {
 ...
 */
export const consumeEvent = async (exchange, routingKey, queueName, onMessage) => {
    if (!channel)
        throw new Error('RabbitMQ channel not initialized');
    await channel.assertExchange(exchange, 'topic', { durable: true });
